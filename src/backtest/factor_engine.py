...
Key differences vs. BacktestEngine:
  ...
  - Signals are AlphaSignal objects (z-score weighted, not discrete)
...
class FactorBacktestEngine:
    ...
    buy_threshold : float
        Minimum alpha score to trigger BUY (default 0.2).
    sell_threshold : float
        Maximum alpha score to trigger SELL (default -0.2).
    factors : list[BaseFactor], optional
        Factor instances to use.  Defaults to all registered technical factors
        (auto-imports src.factors.technical on first use).
    ...
    def run(self) -> BacktestResult:
        ...
        # Position tracking state for risk manager
        ...
        for i in range(n):
            ...
            # ── Risk check on existing position ──────────────────────────
            ...
            alpha: AlphaSignal = self._combiner.combine(factor_results, regime=regime, timestamp=ts)
            ...
            if signal != "HOLD" and self.portfolio.position == 0:
                ...
                # Direction reversal: close existing and flip
                is_long = self.portfolio.position > 0
                should_flip = (signal == "SELL" and is_long) or (signal == "BUY" and not is_long)
                if should_flip:
                    trade = self.portfolio.force_close(price, ts, i)
                    if trade is not None:
                        trade.symbol = self.symbol
        ...
        final_equity = equity_series.iloc[-1] if len(equity_series) else self.initial_capital
